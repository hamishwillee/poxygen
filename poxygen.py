...
"""
This Python script converts DOxygen XML output from C++ into gitbook-compatible markdown. 
...
Other documentation can be found in: https://github.com/hamishwillee/poxygen
"""
...
parser.add_argument("-d", "--debug", default='',help="Include strings for various objects as HTML comments in output.")
...
if not os.path.exists(DOXYGEN_ROOT_DIR):
    print('Docs directory does not exist: %s' % DOXYGEN_ROOT_DIR)
    exit()
if not os.path.exists(DOXYGEN_XML_DIR):
    print('XML directory does not exist (created by doxygen): %s' % DOXYGEN_XML_DIR)
    exit()
